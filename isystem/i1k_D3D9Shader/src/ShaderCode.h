...
"};"
/*
"float3 DrawScene(float3 wp, float3 rD, int numSamples, int reflectPass)"
"{"
...
"uniform float3 EyePos : register(C4), uniform float2 Planes : register(C5), uniform float3 LightPos : register(C7)"
"):color"
"{"
	"float3 rD = normalize(Ray);"
    "float t=999;"
	...
	"{"
		...
		"numIter++;"
	"}"
    "float tNew = length(samplePos-EyePos);"

	"float3 N; float foo; N.x = distFn(samplePos+float3(EPS,0,0), foo)-distFn(samplePos-float3(EPS,0,0), foo); N.y = distFn(samplePos+float3(0,EPS,0), foo)-distFn(samplePos-float3(0,EPS,0), foo); N.z = distFn(samplePos+float3(0,0,EPS), foo)-distFn(samplePos-float3(0,0,EPS), foo); N = normalize(N);"
	//"float3 N = normalize(cross(ddy(rD*tNew),ddx(rD*tNew)));"
	"float3 L = normalize(LightPos);"
	"float m = 999;"
	"float3 shadowPos = samplePos+L*Planes.x;"
	"float shadowAttrib=-1;"
	...
	"float n = 999;"
	"float3 reflectPos = samplePos+R*Planes.x;"
	"float reflectAttrib=-1;"
	"for(int i=0;(i<REFLECTSAMPLES && (abs(n) > EPS)); i++)"
	"{"
		"n = distFn(reflectPos, reflectAttrib);"
		"reflectPos += n*R;"
	"}"
	"float3 H = normalize(L-rD);"
	"float spec = pow(max(0.0, dot(N,H)), 50) + reflectAttrib*0.1;"
	"float aoFoo;"
	"float aoTerm = saturate(distFn(samplePos+N*AO_PLANE, aoFoo)*1.27);"
